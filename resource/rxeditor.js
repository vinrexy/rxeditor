(function () {
	...
	function removeWhiteSpace(root) {
		...
		while (node) {
			...
			if (nodeType === TEXT_NODE) {
				...
				while (hasClass(previous, 'rxeditor-ignore')) {
				...
				if (isInline(node) && previous) {
					...
					while (previousSibling.lastChild) {
						...
						while (hasClass(previousSibling, 'rxeditor-ignore')) {
	...
	/**
	 * Default options for RXEditor
	 * @type {Object}
	 */
	var defaultOptions = {
		/** @lends jQuery.rxeditor.defaultOptions */
		/**
		 ...
		 * @type {string}
		 */
		style: 'jquery.rxeditor.default.css',
		 ...
		 * mode.
		 *
		 * See RXEditor.BBCodeParser.defaults for list of valid options
		 *
		 * @type {Object}
		 */
		parserOptions: { },
	...
	var _templates = {
		...
		toolbarButton: '<a class="rxeditor-button rxeditor-button-{name}" ' +
			'data-rxeditor-command="{name}" unselectable="on">' +
			'<div unselectable="on">{dispName}</div></a>',

		emoticon: '<img src="{url}" data-rxeditor-emoticon="{key}" ' +
			'alt="{key}" title="{tooltip}" />',

		fontOpt: '<a class="rxeditor-font-option" href="#" ' +
			'data-font="{font}"><font face="{font}">{font}</font></a>',

		// sizeOpt: '<a class="rxeditor-fontsize-option" data-size="{size}" ' +
		// 	'href="#"><font size="{size}">{size}</font></a>',
		
		sizeOpt: '<a class="rxeditor-fontsize-option" data-size="{value}px" ' + // hack
			'href="#"><font style="font-size:{value}px">{size}</font></a>',
	...
	/**
	 * Map of all the commands for RXEditor
	 * @type {Object}
	 * @name commands
	 * @memberOf jQuery.rxeditor
	 */
	var defaultCmds = {
		// START_COMMAND: Bold
		bold: {
		...
		on: {
			_dropDown: function (editor, caller, callback) {
				...
				// for (var i = 30; i <= 50; i += 5) { // hack
				// 	appendChild(content, _tmpl('sizeOpt', {
				// 		size: i, value:i-10
				// 	}, true));
				// }
				// editor.createDropDown(caller, 'on', content);
			},
			exec: function (caller) {
				var editor = this;
				// defaultCmds.on._dropDown(editor, caller, function (fontSize) {
				// 	var selection = editor.getDocument().getSelection().toString()
				// 	console.log('selection', selection, fontSize);
				// 	var fontString = `<on style="font-size:${fontSize}">${selection}</on>`;
				// 	editor.execCommand('insertHtml', fontString);
				// });
			},
			tooltip: 'On Click'
		},
		...
		size: {
			...
			exec: function (caller) {
				...
				defaultCmds.size._dropDown(editor, caller, function (fontSize) {
					...
					var selection = editor.getDocument().getSelection().toString()
					console.log('selection', selection, fontSize);
					var fontString = `<font style="font-size:${fontSize}">${selection}</font>`;
					editor.execCommand('insertHtml', fontString);
				});
			},
			tooltip: 'Font Size'
			...
			_dropDown: function (editor, caller, callback) {
				...
				if (!cmd._htmlCache) {
					editor.opts.colors.split('|').forEach(function (column) {
						html += '<div class="rxeditor-color-column">';

						column.split(',').forEach(function (color) {
							html +=
								'<a href="#" class="rxeditor-color-option"' +
	...
	function RangeHelper(win, d, sanitize) {
		var	_createMarker, _prepareInput,
			...
			startMarker  = 'rxeditor-start-marker',
			endMarker    = 'rxeditor-end-marker',
			base         = this;
		...
		_createMarker = function (id) {
			...
			var marker  = createElement('span', {
				id: id,
				className: 'rxeditor-selection rxeditor-ignore',
		...
		base.selectRange = function (range) {
			...
			if (range.collapsed && container &&
				...
				while (lastChild && is(lastChild, '.rxeditor-ignore')) {
	...
	function checkWhitespace(node, rangeHelper) {
		...
		var emoticons = node && find(node, 'img[data-rxeditor-emoticon]');
		...
		for (var i = 0; i < emoticons.length; i++) {
			...
			previousText += data(emoticon, 'rxeditor-emoticon');
	...
	function wrapInlines(body, doc) {
		...
		traverse(body, function (node) {
			if (isInline(node, true)) {
				...
				// Ignore rxeditor-ignore elements unless wrapping siblings
				// Should still wrap both if wrapping siblings.
				if (wrapper || node.nodeType === TEXT_NODE ?
					/\S/.test(node.nodeValue) : !is(node, '.rxeditor-ignore')) {
					if (!wrapper) {
	...
	/**
	 * RXEditor - A lightweight WYSIWYG editor
	 *
	 ...
	 * @name RXEditor
	 */
	function RXEditor(original, userOptions) {
		/**
		 * Alias of this
		 *
		 * @private
		...
		function sanitize(html) {
			...
			return html
			// return domPurify.sanitize(html, { // hack
		...
		init = function () {
			...
			var FormatCtor = RXEditor.formats[options.format];
		...
		initLocale = function () {
			...
			locale = RXEditor.locale[options.locale];

			if (!locale) {
				lang   = options.locale.split('-');
				locale = RXEditor.locale[lang[0]];
			}
		...
		initToolBar = function () {
			...
			toolbar = createElement('div', {
				className: 'rxeditor-toolbar',
				unselectable: 'on'
			});

			if (options.icons in RXEditor.icons) {
				icons = new RXEditor.icons[options.icons]();
			}

			each(groups, function (_, menuItems) {
				group = createElement('div', {
					className: 'rxeditor-group'
				});
		...
		initResize = function () {
			var	minHeight, maxHeight, minWidth, maxWidth,
				...
				grip        = createElement('div', {
					className: 'rxeditor-grip'
				}),
				// Cover is used to cover the editor iframe so document
				// still gets mouse move events
				cover       = createElement('div', {
					className: 'rxeditor-resize-cover'
				}),
		...
		base.maximize = function (maximize) {
			var maximizeSize = 'rxeditor-maximize';
		...
		base.createDropDown = function (menuItem, name, content) {
			...
			dropdown = createElement('div', {
				className: 'rxeditor-dropdown ' + dropDownClass
			});
		...
		base.wysiwygEditorInsertHtml = function (
			...
			marker   = find(wysiwygBody, '#rxeditor-end-marker')[0];
			show(marker);
		...
		appendNewLine = function () {
			...
			rTraverse(wysiwygBody, function (node) {
				...
				if (node.nodeType === ELEMENT_NODE &&
					...
					if (!is(node, '.rxeditor-nlf') && hasStyling(node)) {
						var paragraph = createElement('p', {}, wysiwygDocument);
						paragraph.className = 'rxeditor-nlf';
		...
		base.emoticons = function (enable) {
			...
			if (enable) {
				...
				var emoticons =
					find(wysiwygBody, 'img[data-rxeditor-emoticon]');

				each(emoticons, function (_, img) {
					var text = data(img, 'rxeditor-emoticon');
		...
		triggerValueChanged = function (saveRange) {
			...
			// Don't need to save the range if rxeditor-start-marker
			// is present as the range is already saved
			saveRange = saveRange !== false &&
				!wysiwygDocument.getElementById('rxeditor-start-marker');
	...
	/**
	 * Map containing the loaded RXEditor locales
	 * @type {Object}
	 * @name locale
	 * @memberOf rxeditor
	 */
	 ...
	 * @class command
	 * @name rxeditor.command
	 */
	RXEditor.command =
	/** @lends rxeditor.command */
	{
		/**
		 * Gets a command
		 *
		 ...
		 * {@link rxeditor.execCommand}</p>
		 *
		 * @example
		 * RXEditor.command.set("hello",
		 * {
		 *     exec: function () {
		 *         alert("Hello World!");
		 *     }
	...
	/**
	 * RXEditor
	 * http://www.rxeditor.com/
	 *
	 * Copyright (C) 2017, Vinrexy (appdance.cn)
	 *
	 * RXEditor is licensed under the MIT license:
	 *	http://www.opensource.org/licenses/mit-license.php
	 *
	 * @fileoverview RXEditor - A lightweight WYSIWYG BBCode and HTML editor
	 * @author Vinrexy
	 */


	window.rxeditor = {
		command: RXEditor.command,
		...
		locale: RXEditor.locale,
		icons: RXEditor.icons,
		utils: {
			each: each,
		...
		formats: RXEditor.formats,
		create: function (textarea, options) {
			options = options || {};

			// Don't allow the editor to be initialised
			// on it's own source editor
			if (parent(textarea, '.rxeditor-container')) {
				return;
			}

			if (options.runWithoutWysiwygSupport || isWysiwygSupported) {
				/*eslint no-new: off*/
				(new RXEditor(textarea, options));
			}
		},
		instance: function (textarea) {
			return textarea._rxeditor;
		}
	};
