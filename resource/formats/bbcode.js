/**
 * RXEditor Cocos RichText Plugin
 * http://www.rxeditor.com/
 *
 * Copyright (C) 2022-2024, vinrexy (appdance.cn)
 *
 * RXEditor is licensed under the MIT license:
 *	http://www.opensource.org/licenses/mit-license.php
 *
 * @fileoverview RXEditor BBCode Format
 * @author Vinrexy
 */
(function (rxeditor) {
	...
	var escapeEntities  = rxeditor.escapeEntities;
	var escapeUriScheme = rxeditor.escapeUriScheme;
	var dom             = rxeditor.dom;
	var utils           = rxeditor.utils;
	...
	var EMOTICON_DATA_ATTR = 'data-rxeditor-emoticon';

	var getEditorCommand = rxeditor.command.get;
	...
	var bbcodeHandlers = {
		...
		on: {
			...
			format: function (element, content) {
				var	click = attr(element, 'click')
				var	param = attr(element, 'param')
				return `<on click="${click}" param="${param}">${content}</on>`
			},
			html: function (token, attrs, content) {
				// console.log('on html', attrs, content)
				return `<on click="${attrs.click}" param="${attrs.param}">${content}</on>`
			}
		},
		...
		size: {
			...
			format: function (element, content) {
				...
				console.log('size fomat fontSize=', fontSize)
			...
			html: function (token, attrs, content) {
				return `<font style="font-size:${attrs.defaultattr-10}px">${content}</font>`
			}
		},
		// END_COMMAND
		...
		img: {
			...
			format: function (element, content) {
				...
				return '<img' + ' src="' + attr(element, 'src') + '"' + attribs + ' />';
			},
			html: function (token, attrs, content) {
				...
				return content
				// return '<img' + ' src="' + escapeUriScheme(content) + '"' + attribs +' />'; // hack
			}
		},
		// END_COMMAND
	...
	function bbcodeFormat() {
		 ...
		 * @memberOf RXEditor.plugins.bbcode.prototype
		 */
			...
			var toBBCode = function (node, hasCodeParent, vChildren) {
				...
				dom.traverse(node, function (node) {
					...
					if (nodeType === 1) {
						...
						if (is(node, '.rxeditor-nlf') && !firstChild) {
							return;
						}
		...
		function toSource(asFragment, html, context, parent) {
			...
			// Remove all nodes with rxeditor-ignore class
			elements = container.getElementsByClassName('rxeditor-ignore');
	...
	rxeditor.formats.bbcode = bbcodeFormat;
	rxeditor.BBCodeParser = BBCodeParser;
}(rxeditor));
