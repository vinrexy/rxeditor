...
(function (rxeditor, $) {
	...
	var oldSet = rxeditor.command.set;
	rxeditor.command.set = function (name, cmd) {
	...
	var oldCreate = rxeditor.create;
	rxeditor.create = function (textarea, options) {
		oldCreate.call(this, textarea, options);

		if (textarea && textarea._rxeditor) {
			var editor = textarea._rxeditor;
