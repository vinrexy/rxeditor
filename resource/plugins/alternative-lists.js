/**
 * RXEditor Inline-Code Plugin for BBCode format
 * http://www.rxeditor.com/
 *
 * Copyright (C) 2011-2013, Vinrexy (appdance.cn)
 *
 * RXEditor is licensed under the MIT license:
 *	http://www.opensource.org/licenses/mit-license.php
 *
 * @fileoverview RXEditor alternative lists plugin
...
(function (rxeditor) {
	...
	rxeditor.plugins['alternative-lists'] = function () {
		var base = this;
		...
		base.init = function () {
			...
			rxeditor.command.get('orderedlist').txtExec = orderedHandler;
			rxeditor.command.get('bulletlist').txtExec = bulletHandler;

			// Override current implementation
			rxeditor.formats.bbcode.set('list', {
				breakStart: true,
				isInline: false,
				...
				html: function (token, attrs, content) {
					...
					if (listType === '1') {
						...
						toHtml = rxeditor.formats.bbcode.get('ol').html;
					} else {
						// unknown listType, use default bullet list behavior
						toHtml = rxeditor.formats.bbcode.get('ul').html;
					}
					...
					if (isFunction(toHtml)) {
						...
						return rxeditor.formats.bbcode.formatBBCodeString(
							toHtml, token.attrs);
					}
				}
			});

			rxeditor.formats.bbcode.set('ul', {
				tags: {
					ul: null
				},
				breakStart: true,
			...
			rxeditor.formats.bbcode.set('ol', {
				tags: {
					ol: null
				},
				breakStart: true,
			...
			rxeditor.formats.bbcode.set('li', {
				tags: {
					li: null
				},
				isInline: false,
